...
BOOST_AUTO_TEST_CASE( wasm_interrupt_test ) { try {
   ...
   // eosvmoc. That action will then fail when it hits the 60000ms deadline.
   // 60000ms has to be long enough for oc compile to complete and kill the non-oc executing transaction
   BOOST_CHECK_THROW( push_trx( t, test_api_action<WASM_TEST_ACTION("test_checktime", "checktime_failure")>{},
                                0, 150, 60000, true, fc::raw::pack(10000000000000000000ULL) ),
                      deadline_exception );
   ...
   // if post_id == pre_id + 1, then likely that 60000ms above was not long enough for oc compile to complete
