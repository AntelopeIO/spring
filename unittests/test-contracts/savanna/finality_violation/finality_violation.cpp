...
std::pair<std::string, std::string> finality_violation::addviolation(const finalizer_policy_input& finalizer_policy, const proof& proof1, const proof& proof2 ){
    ...
    check ( block_finality_data_internal(proof1.target_block_proof_of_inclusion.target.finality_data).finality_digest()
            != block_finality_data_internal(proof2.target_block_proof_of_inclusion.target.finality_data).finality_digest() , "proof target blocks must have a different finality digest");
