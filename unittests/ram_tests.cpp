...
BOOST_AUTO_TEST_CASE_TEMPLATE( ram_tests, T, eosio_system_testers ) { try {
   ...
   for (auto i = 0; i < 10; ++i) {
      try {
         chain.set_code( "testram11111"_n, test_contracts::test_ram_limit_wasm() );
         break;
   ...
   for (auto i = 0; i < 10; ++i) {
      try {
         chain.set_abi( "testram11111"_n, test_contracts::test_ram_limit_abi() );
         break;
      } catch (const ram_usage_exceeded&) {
         init_request_bytes += increment_contract_bytes;
         chain.buyrambytes(config::system_account_name, "testram11111"_n, increment_contract_bytes);
   ...
   chain.produce_block();
   chain.set_code( "testram22222"_n, test_contracts::test_ram_limit_wasm() );
   chain.set_abi( "testram22222"_n, test_contracts::test_ram_limit_abi() );
   chain.produce_block();

   auto total = chain.get_total_stake( "testram11111"_n );
   ...
   total = chain.get_total_stake( "testram11111"_n );
