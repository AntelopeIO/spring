   ...
   }
   {
      ...
      auto fill_keys = [](const flat_set <permission_level>& provided_permissions, flat_set <public_key_type>& provided_keys) {
         ...
         for (auto &permission : provided_permissions) {
            ...
            provided_keys.insert(public_key_type{sig, fc::sha256{digest}, fc::check_canonical_t::no});
         }
      };
...
BOOST_AUTO_TEST_CASE_TEMPLATE( transaction_test, T, validating_testers ) { try {
   ...
   (void)((const signed_transaction &)trx).sign( test.get_private_key( config::system_account_name, "active" ), test.get_chain_id());
   BOOST_CHECK_EQUAL(0u, trx.signatures.size());
