...
BOOST_AUTO_TEST_CASE_TEMPLATE(action_receipt_tests, T, validating_testers) { try {
   ...
   auto call_doit_and_check = [&]( account_name contract, account_name signer, auto&& checker ) {
      ...
      trx.sign( chain.get_private_key(signer, "active"), chain.control->get_chain_id() );
   ...
   chain.produce_blocks(1); // Added to avoid the last doit transaction from being considered a duplicate.
...
BOOST_AUTO_TEST_CASE_TEMPLATE(action_tests, T, validating_testers) { try {
   ...
   auto test_require_notice = [&chain](auto& test, std::vector<char>& data, std::vector<account_name>& scope){
      ...
      test.set_transaction_headers(trx);
      trx.sign(test.get_private_key("inita"_n, "active"), chain.control->get_chain_id());
      auto res = test.push_transaction(trx);
      BOOST_CHECK_EQUAL(res->receipt->status, transaction_receipt::executed);
   };
   ...
   auto a3a4_scope = std::vector<account_name>{"acc3"_n, "acc4"_n};
   {
      ...
      trx.sign(chain.get_private_key("testapi"_n, "active"), chain.control->get_chain_id());
      trx.sign(chain.get_private_key("acc3"_n, "active"), chain.control->get_chain_id());
      trx.sign(chain.get_private_key("acc4"_n, "active"), chain.control->get_chain_id());
      auto res = chain.push_transaction(trx);
      BOOST_CHECK_EQUAL(res->receipt->status, transaction_receipt::executed);
   }
...
BOOST_AUTO_TEST_CASE_TEMPLATE(cf_action_tests, T, validating_testers) { try {
      ...
      sigs = trx.sign(chain.get_private_key("testapi"_n, "active"), chain.control->get_chain_id());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(cfa_tx_signature, T, validating_testers)  try {
   ...
   BOOST_TEST(tx1.sign(priv_key, chain.control->get_chain_id()).to_string() != tx2.sign(priv_key, chain.control->get_chain_id()).to_string());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(resource_limits_tests, T, validating_testers) {
   ...
   auto pushit = [&]{
      ...
      chain.set_transaction_headers(trx);
      trx.sign(chain.get_private_key( "rlimits"_n, "active" ), chain.control->get_chain_id());
      chain.push_transaction(trx);
   };
   pushit();
   chain.produce_block();
