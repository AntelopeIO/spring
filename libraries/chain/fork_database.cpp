...
namespace eosio::chain {
   ...
   struct fork_database_impl {
      ...
      bool             child_of_pending_savanna_lib_impl(const block_id_type& id) const;
      bool             is_child_of_impl(const block_id_type& ancestor, const block_id_type& descendant) const;
   ...
   template<class BSP>
   bool fork_database_t<BSP>::child_of_pending_savanna_lib( const block_id_type& id ) const {
      std::lock_guard g( my->mtx );
      return my->child_of_pending_savanna_lib_impl(id);
   }

   template<class BSP>
   bool fork_database_impl<BSP>::child_of_pending_savanna_lib_impl(const block_id_type& id) const {
      if (pending_savanna_lib_id == id)
         return true;
      return is_child_of_impl(pending_savanna_lib_id, id);
   }

   template<class BSP>
   bool fork_database_t<BSP>::is_child_of(const block_id_type& ancestor, const block_id_type& descendant) const {
      std::lock_guard g( my->mtx );
      return my->is_child_of_impl(ancestor, descendant);
   }

   template<class BSP>
   bool fork_database_impl<BSP>::is_child_of_impl(const block_id_type& ancestor, const block_id_type& descendant) const {
      ...
      for (; i != index.end(); i = index.find((*i)->previous())) {
         if ((*i)->previous() == ancestor)
            return true;
         if ((*i)->block_num() <= ancestor_block_num+1)
            return false;
      }
