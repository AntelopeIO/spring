...
BOOST_AUTO_TEST_CASE(good) try {
   ...
   BOOST_CHECK_EQUAL(wa_pub, make_webauthn_sig(priv, auth_data, json).recover(d, check_canonical_t::yes));
} FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE(mismatch_presence) try {
   ...
   BOOST_CHECK_NE(wa_pub, make_webauthn_sig(priv, auth_data, json).recover(d, check_canonical_t::yes));
} FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE(non_https) try {
   ...
   BOOST_CHECK_EXCEPTION(make_webauthn_sig(priv, auth_data, json).recover(d, check_canonical_t::yes), fc::assert_exception, [](const fc::assert_exception& e) {
...
BOOST_AUTO_TEST_CASE(challenge_junk) try {
   ...
   BOOST_CHECK_EXCEPTION(make_webauthn_sig(priv, auth_data, json).recover(d, check_canonical_t::yes), fc::exception, [](const fc::exception& e) {
...
BOOST_AUTO_TEST_CASE(damage_sig) try {
   ...
   try {
      recovered_pub = sig.recover(d, check_canonical_t::yes);
...
BOOST_AUTO_TEST_CASE(different_priv_key) try {
   ...
   BOOST_CHECK_NE(wa_pub, make_webauthn_sig(other_priv, auth_data, json).recover(d, check_canonical_t::yes));
} FC_LOG_AND_RETHROW();
